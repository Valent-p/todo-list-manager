...
function App() {
  ...
  const [items, setItems] = useState(() => [
    { id: 1, title: 'A', isDone: false, doneNote: '', description: 'A sample task.' },
    { id: 2, title: 'B', isDone: true, doneNote: 'A done work.', description: 'A sample task.' },
    { id: 3, title: 'C', isDone: false, doneNote: '', description: 'A sample task.' },
  ]);

  const nextId = useRef(4);

  const done_items = items.filter(item => item.isDone);
  const not_done_items = items.filter(item => !item.isDone);
  ...
  function addItem({title, description}){
    const newItem = { id: nextId.current++, title, description, isDone:false, doneNote: '' };
    setItems(prev => [newItem, ...prev]);
  }

  function toggleItem(id){
    setItems(prev => prev.map(it => it.id === id ? {...it, isDone: !it.isDone} : it));
  }
